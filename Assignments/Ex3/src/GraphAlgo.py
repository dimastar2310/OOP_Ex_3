...
class GraphAlgo(GraphAlgoInterface):
    ...
    def get_graph(self) -> Type[DiGraph]:
        return self.graph
    ...
    def isConnected(self) -> bool:
        ...
        for v in all_nodes:
            ...
            for v in all_nodes2:
                ...
                if v.getTag() != 1: #asiti tiul ba graph be rega she ehad mihehem le 1 az false
                    return False
    ...
    def shortest_path(self, id1: int, id2: int) -> (float, list):
        ...
        start: MyNodeData = g.get_v(key=id1)
        start.setWeight(0)

        pq.put(start)
        while not pq.empty():

            n1: MyNodeData = pq.get()

            edges_of_n = g.all_in_edges_of_node(n1.getkey()).items()

            for dest, weight in edges_of_n:

                n2: MyNodeData = g.get_v(dest)
    ...
    def plot_graph(self) -> None:
        ...
        for src in g.get_all_v():
            for dest in g.all_in_edges_of_node(src):
                ...
                plt.annotate("", xy=(float(l1[0]), float(l1[1])), xytext=(l2[0], l2[1]),
                             arrowprops=dict(arrowstyle="->"))
                plt.text(((l1[0] + l2[0]) / 2), ((l1[1] + l2[1]) / 2),
                         str(format(g.all_in_edges_of_node(src).get(dest), ".2f")), color="green", fontsize="10")
        plt.show()
